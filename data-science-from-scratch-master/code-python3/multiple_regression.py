...
def squared_error_gradient(x_i, y_i, beta):
    """i번째 오류 제곱 값의 beta에 대한 편미분값"""
    return [-2 * x_ij * error(x_i, y_i, beta)
            for x_ij in x_i]

def estimate_beta(x, y):
    """beta_initial: 임의의 beta
    minimize_stochastic: Gradient descent를 활용하는 함"""
...
def multiple_r_squared(x, y, beta):
    """multipe regression에서의 결정계수값"""
    sum_of_squared_errors = sum(error(x_i, y_i, beta) ** 2
                                for x_i, y_i in zip(x, y))
    return 1.0 - sum_of_squared_errors / total_sum_of_squares(y)

def bootstrap_sample(data):
    """data의 개수만큼 중복을 허용한 무작위 재추출"""
    return [random.choice(data) for _ in data]

def bootstrap_statistic(data, stats_fn, num_samples):
    """num_samples개의 bootstrap_sample return에서의 추출값에 대해 stats_fn을 적용"""
    return [stats_fn(bootstrap_sample(data))
            for _ in range(num_samples)]

def estimate_sample_beta(sample):
    x_sample, y_sample = list(zip(*sample)) # zip함수!
...
#
# alpha: 페널티의 강도를 조절하는 하이퍼파라미터
# 앤드류 응의 강의에서는 lambda로 표현하지만 파이썬에서는 lambda 함수를 사용하므로 alpha로 표현
def ridge_penalty(beta, alpha):
  return alpha * dot(beta[1:], beta[1:])

def squared_error_ridge(x_i, y_i, beta, alpha):
    """beta 값을 활용할 때의 오류와 ridge 페널티의 합"""
...
def squared_error_ridge_gradient(x_i, y_i, beta, alpha):
    """i번 오류 제곱값과 페널티 합의 편미분값"""
    return vector_add(squared_error_gradient(x_i, y_i, beta),
                      ridge_penalty_gradient(beta, alpha))

def estimate_beta_ridge(x, y, alpha):
    """페널티가 alpha일 때의 ridge 회귀를 gradient descent로 학습"""
    beta_initial = [random.random() for x_i in x[0]]
